...
/** @type {Object|null} Device data from /api/device/{part} */
let deviceData = null;
...
async function loadDevice(pkg) {
    ...
    try {
        ...
        if (!resp.ok) {
            ...
            setStatus(err.detail || 'Device not found');
            return;
        }
...
async function generateCode() {
    ...
    try {
        ...
        document.getElementById('code-output').textContent = '// Error generating code: ' + e.message;
    }
}
...
/** Download all generated files (pin_config.c and pin_config.h). */
function exportCode() {
...
/** Export the current configuration (assignments, signals, osc, fuses) as a JSON file. */
function saveConfig() {
    if (!deviceData) return;
    const config = {
        part_number: deviceData.part_number,
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `${deviceData.part_number}_${deviceData.selected_package}.json`;
    a.click();
    URL.revokeObjectURL(url);
}

/**
 * Import a previously saved configuration JSON file.
 * Restores all state (assignments, signals, oscillator, fuses) and reloads the device.
...
async function loadConfig(file) {
    try {
        ...
        setStatus(`Loaded config: ${config.part_number} — ${config.package || 'default'}`);
    } catch (e) {
...
async function checkCompiler() {
    try {
        const resp = await fetch('/api/compiler');
        const data = await resp.json();
...
async function compileCheck() {
    ...
    try {
        const resp = await fetch('/api/compile-check', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                code: generatedFiles['pin_config.c'] || code,
                header: generatedFiles['pin_config.h'] || '',
                part_number: deviceData.part_number,
            }),
        });
        ...
        if (!resp.ok) {
            ...
            resultBox.textContent = err.detail || 'Compile check failed';
            return;
        }
